    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="fixed bottom-6 right-4 z-[100] md:hidden flex flex-col items-end gap-2"
        >
          {/* Dismiss button */}
          <button
          ...
          <motion.a
            ...
            aria-label={`Jetzt anrufen: ${electricianConfig.phone}`}
          >
            <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center shrink-0">
              <Phone size={16} fill="white" />
            </div>
            <div className="text-left">
              <div className="text-[10px] text-white/70 leading-none mb-0.5">Notdienst 24/7</div>
              <div className="font-bold leading-none">{electricianConfig.phone}</div>
            </div>
          </motion.a>
        </motion.div>
      )}
